...
import { KafkaConnectionOptions } from "../src/kafka/interfaces/kafka-connection-options";
...
describe("KafkaService", () => {
  ...
  describe("features", () => {
    ...
    it("should not get consumer", async () => {
      await initModule({});
      expect(() => service.getConsumer()).toThrow(
    ...
    it("should get not producer", async () => {
      await initModule({});
      expect(() => service.getProducer()).toThrow(
    ...
    describe("App consuming KafkaModule", () => {
      it("should consume message", async () => {
        await initModule();
        runSubscribeTopic(service);
        expect(await service.consumeMessage(null, mockKafkaMessages)).not
          .toThrow;
      });
      ...
      it("should consume message (error)", async () => {
        ...
        runSubscribeTopic(service);
        await expect(
          service.consumeMessage(new Error(), mockKafkaMessages),
        ).rejects.toThrow();
      });
    });
